...
export default function PropertiesJournal({ onBookClick }: PropertiesJournalProps) {
  ...
  // Map the rich properties data to the required card format
  const properties = propertiesData.map((p) => ({
    id: p.id,
    title: p.title,
    location: p.nearby[0]?.name || "Exclusive Location",
    price: `₹${p.price.toLocaleString()}`,
    rating: p.reviews[0]?.rating || 4.9,
    guests: p.stats.guests,
    image: p.coverImage,
    tag: p.id === "tuscan" ? "Luxury Villa" : "Retreat"
  }));
            ...
            >
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-stone-400 text-xs md:text-sm">from <span className="text-accent font-medium">{property.price}</span></p>
                </div>
              </div>
